...
function remover() {
  var selected = Array
    .from(document.querySelectorAll('.qc_item_checks:checked'))
    .map(_.property('value'));

  if(!selected.length) {
    swal('Escolha um ou mais itens para remover', '', 'error');

    return false;
  }

  swal({
    title: 'Remover Insumos',
    text: 'Tem certeza que deseja remover estes insumos do Quadro de Concorrência?',
    showCancelButton: true,
    confirmButtonColor: "#DD6B55",
    confirmButtonText: "Sim, tenho certeza!",
    cancelButtonText: "Não",
    ...
    var ajax = $.post('/quadro-de-concorrencia/remover-itens', {
      itens: selected
    });
    ...
    ajax.fail(function(xhr) {
      var message = xhr.responseJSON ? xhr.responseJSON.message : false;
      swal(
        'Ops!',
        message || 'Não foi possível remover os itens do Quadro de Concorrência',
        'error'
      );
    });
  });
