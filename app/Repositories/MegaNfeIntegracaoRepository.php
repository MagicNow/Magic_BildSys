...
class MegaNfeIntegracaoRepository
{
    ...
    public static function integra($id)
    {
        ...
        foreach ($notafiscal->itens as $k => $item) {
            foreach ($item->solicitacaoEntregaItens as $itemSolicitacao) {
                $ajustesPrecificacao[$item->id]['itensSolicitacao'][] = $itemSolicitacao;
            }
        }
        ...
        foreach ($notafiscal->itens as $k => $item) {
            foreach($item->solicitacaoEntregaItens as $itemSolicitacao) {
                ...
                if (count($ajustesPrecificacao[$k]) > 1) {
                    $precificacaoAjustada = self::calculaPrecificacao($item, $ajustesPrecificacao[$k]);
                    if ($precificacaoAjustada) {
                        //TODO: Atribui valor percentual do $item na NF ao $itemSolicitacao referente.
                        
                    }
                }

                $codigoInsumo = $itemSolicitacao->insumo->codigo;
                ...
                $item = [
                    ...
                    'ITENS_UNI_ST_UNIDADE' => $itemSolicitacao->insumo->unidade_sigla,
    ...
    public static function calculaPrecificacao($item, $itemsSolicitacao)
    {
        //TODO: Calcular valores por item vs items solicitacoes Associados.
