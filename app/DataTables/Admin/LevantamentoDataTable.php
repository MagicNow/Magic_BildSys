    ...
    public function ajax()
    {
        return $this->datatables
            ...
            ->filterColumn('data_inicio', function ($query, $keyword) {
                $query->whereRaw("DATE_FORMAT(levantamentos.data_inicio,'%d/%m/%Y') like ?", ["%$keyword%"]);
            })
            ...
            ->filterColumn('data_termino', function ($query, $keyword) {
                $query->whereRaw("DATE_FORMAT(levantamentos.data_termino,'%d/%m/%Y') like ?", ["%$keyword%"]);
            })
            ->make(true);
    }
    ...
    public function query()
    {
        ...
        $levantamentos = Levantamento::query()
            ->select([
				...
				'levantamentos.perda',
				'levantamentos.data_inicio',
				'levantamentos.data_termino',
				'levantamentos.created_at'					
            ])
        ->join('obras','obras.id','levantamentos.obra_id')
		->join('insumos','insumos.id','levantamentos.insumo_id');		
