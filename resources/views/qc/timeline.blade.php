@if($emAprovacao)
<div class="panel panel-default panel-body pointer" id="linhaDoTempo" data-id="{{ $qc->id }}" data-workflow-tipo="7">
    <h4 class="highlight">
        Timeline
    ...
    @if($alcadas_count)
        ...
        <h4 class="col-md-{{$col_md}} col-sm-{{$col_md}}" style="padding-right: 1px;padding-left: 1px;"> <span>
                Criação
                <small>{{ $qc->created_at->format('d/m/Y H:i') }}</small>
            </span>
            <div class="progress">
                    ...
                    <h4 class="col-md-{{$col_md}} col-sm-{{$col_md}}" style="padding-right: 1px;padding-left: 1px;">
                        ...
                        @if($count == $alcadas_count)
                            <span class="pull-right">
                                Finalizada
                                <small>{{ $qc->updated_at->format('d/m/Y H:i') }}</small>
                            </span>
                        @endif
...
@else
    <div class="well well-sm" id="linhaDoTempo" data-id="{{ $qc->id }}"
        data-workflow-tipo="7">
        <h4 class="highlight">
            Aprovações
            <i class="fa fa-info-circle text-info"></i>
