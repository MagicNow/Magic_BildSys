...
<div>
    <div class="form-group col-sm-6">
        ...
        @else
            {!! Form::select('fornecedor_cod', ['' => 'Escolha...']+$fornecedores,  null,
            ['class' => 'form-control','id'=>'fornecedor_cod','required'=>'required']) !!}
        @endif
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text"
                                   ...
                                   class="form-control" required="required" name="{{ $v_tag }}"
                                   placeholder="{{ $valor->nome }}">
                        </td>
                        <td class="text-center">
                            <label for="{{ $v_tag }}">{{ $valor->tipo }}</label>
                        </td>
            ...
            @foreach ($catalogoContrato->contratoInsumos->sortByDesc('id')->groupBy('insumo_id') as $insumo)
                @foreach($insumo as $item)
                    ...
                    <div class="form-group col-md-12 bloco_insumos_id_{{$item->insumo_id}}">
                        ...
                        <div class="col-md-2"
                             style="margin-top:25px;{{in_array($item->insumo_id, $array_insumos) ? 'display:none;' : ''}}">
                            <div class="col-md-9">
                                <button class="btn btn-success btn-flat btn-block" type="button"
                                        onclick="inserirReajuste({{$item->insumo_id}})">
                                    Reajuste
                                </button>
                            </div>
                        ...
                        <div {{in_array($item->insumo_id, $array_insumos) ?
                        'style=display:none; class=bloco_mostrar_reajustes_'.$item->insumo_id : ''}}>
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text"
                                           {{ $podeEditar?'':' disabled="disabled"' }} value="{{$item->valor_unitario}}"
                                           id="valor_unitario_{{$item->id}}" class="form-control money"
                                           name="contratoInsumos[{{$item->id}}][valor_unitario]">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <label>Pedido quantidade mínima:</label>
                                <input type="text"
                                       {{ $podeEditar?'':' disabled="disabled"' }} value="{{$item->pedido_minimo}}"
                                       id="pedido_minimo_{{$item->id}}" class="form-control money"
                                       name="contratoInsumos[{{$item->id}}][pedido_minimo]">
                            </div>
                            <div class="col-md-2">
                                <label>Pedido múltiplo de:</label>
                                <input type="text"
                                       {{ $podeEditar?'':' disabled="disabled"' }} value="{{$item->pedido_multiplo_de}}"
                                       id="pedido_multiplo_de_{{$item->id}}" class="form-control money"
                                       name="contratoInsumos[{{$item->id}}][pedido_multiplo_de]">
                            </div>
                            <div class="col-md-2">
                                <label>Período início:</label>
                                <input type="date"
                                       {{ $podeEditar?'':' disabled="disabled"' }}
                                       value="{{$item->periodo_inicio ? $item->periodo_inicio->format('Y-m-d') : null}}"
                                       id="periodo_inicio_{{$item->id}}" class="form-control"
                                       name="contratoInsumos[{{$item->id}}][periodo_inicio]">
                            </div>
                            <div class="col-md-2">
                                <label>Período término:</label>
                                <input type="date"
                                       {{ $podeEditar?'':' disabled="disabled"' }}
                                       value="{{$item->periodo_termino ? $item->periodo_termino->format('Y-m-d') : null}}"
                                       id="periodo_termino_{{$item->id}}" class="form-control"
                                       name="contratoInsumos[{{$item->id}}][periodo_termino]">
                            </div>
                        </div>
                    </div>
            ...
            swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            $('#valor_unitario_' + count_reajuste).maskMoney({
                                allowNegative: true,
                                thousands: '.',
                                decimal: ','
                            });
                            $('#pedido_minimo_' + count_reajuste).maskMoney({
                                allowNegative: true,
                                thousands: '.',
                                decimal: ','
                            });
                            $('#pedido_multiplo_de_' + count_reajuste).maskMoney({
                                allowNegative: true,
                                thousands: '.',
                                decimal: ','
                            });
