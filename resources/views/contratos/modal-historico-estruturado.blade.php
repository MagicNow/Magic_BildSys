                ...
                <div class="modal-body">
                    ...
                    <table class="table table-bordered table-all-center">
                        ...
                        @foreach($item->apropriacoes as $apropriacao)
                            {{--{{dump($apropriacao->modificacoes)}}--}}
                            <tr>
                                @if(!isset($impressao))
                                    <td>
                                        <a href="{{ route('contratos.memoria_de_calculo', [$contrato->id, $apropriacao  ->id]) }}"
                                           type="button"
                                ...
                                <td>{{ float_to_money($apropriacao->qtd_sobra, '') }}</td>
                                <td>{{ float_to_money($item->valor_unitario * $apropriacao->qtd_sobra) }}</td>
                                <td>{{ '0,00' }}</td>
                                <td>{{ '0,00' }}</td>
                                <td>{{ float_to_money($apropriacao->qtd_sobra /* - $realizado */, '') }}</td>
                                <td>{{ float_to_money($item->valor_unitario) }}</td>
                                <td>{{ float_to_money(($item->valor_unitario * $apropriacao->qtd_sobra) /* - $realizado */) }}</td>
                                ...
                                <td colspan="10" class="td-full">
                                    @if($apropriacao->contratoItem)
                                        @if($apropriacao->contratoItem->modificacoes->where('contrato_status_id', 2)->isNotEmpty())
                                            @if(count($apropriacao->modificacoes))
